...
The two lines of history had split at `a306c94`. The pin carried 25
commits that `master` did not have — generator, rangeproof (plus its
preallocated variant), surjection proof, and Pedersen commitment
bindings. `libs/common/embit` needs those for Liquid support
(`tests/tests/test_liquid.py`, `src/embit/util/ctypes_secp256k1.py`).

The pin also points its own nested submodule at
`ElementsProject/secp256k1-zkp`, while `master` still points at
`bitcoin-core/secp256k1`, which does not carry the zkp modules at all.
...
files had moved to `8a8fdf2` (`v9.5.0-655`).
...
The three `hwac` forks carry no tags — only branches came across. A local
clone may still name a pin after a tag, as embit does with `v0.8.2` in
`git submodule status`, but that tag came from the original clone and
lives only on disk. Branches are the only refs those forks offer, so
judge their reachability by branch. The two upstream remotes, `lvgl/lvgl`
and `ElementsProject/secp256k1-zkp`, do carry their tags.
...
Write it as an `if`, not as `grep -q ... && { ...; }`. The `&&` form
leaves the step with the exit code of a failed `grep`, so a clean tree
would fail the job.

This is written down only. It is not wired into any workflow.
